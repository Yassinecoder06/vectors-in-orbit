...
INPUT_CSV = DATA_DIR / "lazada-products.csv"
OUTPUT_JSON = DATA_DIR / "lazada_products_payload.json"
...
def process_row(row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        categories_raw = row.get("breadcrumb", "[]")
        categories = parse(categories_raw)
        ...
        payload = {
            ...
            "image_url": parse(row.get("image", ""))[0],
            "region": row.get("country_of_origin", "Singapore"),
