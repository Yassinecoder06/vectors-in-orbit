...
INPUT_CSV = DATA_DIR / "walmart-products.csv"
OUTPUT_JSON = DATA_DIR / "walmart_products_payload.json"
...
def parse(categories_str: str) -> List[str]:
    """
...
def process_row(row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        payload = {
            ...
            "price": price,
            "categories": parse(row.get("categories")),
            "brand": row.get("brand", "Unknown"),
            "in_stock": clean_bool(row.get("in_stock", random.choice(["True", "False"]))),
            "image_url": parse(row.get("image_urls", ""))[0],
            "region": row.get("country_of_origin", "US"),
